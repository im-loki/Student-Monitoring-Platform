...
$usn=$usnErr=$intno=$intnoErr=$Subject1=$Subject1Err=$Subject2=$Subject2Err=$Subject3=$Subject3Err=$Subject4=$Subject4Err=$Subject5=$Subject5Err=$Subject6=$Subject6Err=$comment=$sem=$semErr="";
$flag=1;
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty($_POST["intno"])) {
    $intnoErr = "Intno is required";
    $flag=0;
  } else {
    $intno = strtoupper(test_input($_POST["intno"]));
    // check if name only contains letters and whitespace
    if (!preg_match("/^[1-9]+$/",$intno)) {
      $intnoErr = "Only numbers allowed";
      $flag=0;
    }
  }
  ...
  if($flag==1){
    $msg="in flag";
    echo "inside flag==1";
    $db = mysqli_connect('localhost', 'root', '', 'mar_trial');

    $query = "INSERT INTO trial (usn,intno,subject1,subject2,subject3,subject4,subject5,subject6,sem) 
          VALUES('$usn', '$intno', '$Subject1', '$Subject2','$Subject3','$Subject4','$Subject5','$Subject6','$sem')";
